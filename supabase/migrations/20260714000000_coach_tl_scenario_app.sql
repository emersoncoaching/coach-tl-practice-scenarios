...
create table if not exists public.coach_tl_scenario_submissions (
  ...
  scenario_version text not null default '2026-07-coach-tl-v1',
...
update public.coach_tl_scenario_submissions
...
revoke all on public.coach_tl_scenario_submissions from anon, authenticated;

create or replace function public.submit_coach_tl_scenario(
...
as $$
declare
  inserted public.coach_tl_scenario_submissions;
begin
  ...
  insert into public.coach_tl_scenario_submissions (
    candidate_name,
    candidate_email,
  ...
  values (
    ...
    coalesce(nullif(p_responses->>'scenarioVersion', ''), '2026-07-coach-tl-v1'),
    p_responses,
    p_user_agent
  )
...
create or replace function public.get_coach_tl_submission_for_applicant(
...
as $$
  select
    ...
    responses,
    scenario_version
  from public.coach_tl_scenario_submissions
  where applicant_token = p_applicant_token
  limit 1;
$$;

drop function if exists public.get_coach_tl_submission_for_review(text);

create or replace function public.get_coach_tl_submission_for_review(
  p_review_token text
)
returns table (
  candidate_name text,
...
drop function if exists public.get_coach_tl_submissions_for_admin(text, int);

create or replace function public.get_coach_tl_submissions_for_admin(
  p_admin_token text,
...
as $$
begin
  if encode(extensions.digest(coalesce(p_admin_token, ''), 'sha256'), 'hex') <> '277c8b888a65942507ccd6f40e6f1c2de328b789f0ea3e618420905d0a63d3dd' then
  ...
  return query
    ...
    from public.coach_tl_scenario_submissions as submissions
    order by submissions.created_at desc
    limit least(greatest(coalesce(p_limit, 100), 1), 500);
end;
$$;

drop function if exists public.set_coach_tl_submission_review_status(text, text);

create or replace function public.set_coach_tl_submission_review_status(
  p_review_token text,
...
begin
  ...
  return query
    update public.coach_tl_scenario_submissions as submissions
    set
...
revoke execute on function public.submit_coach_tl_scenario(text, text, jsonb, text, text) from public;
revoke execute on function public.get_coach_tl_submission_for_applicant(text) from public;
revoke execute on function public.get_coach_tl_submission_for_review(text) from public;
revoke execute on function public.get_coach_tl_submissions_for_admin(text, int) from public;
revoke execute on function public.set_coach_tl_submission_review_status(text, text) from public;

grant execute on function public.submit_coach_tl_scenario(text, text, jsonb, text, text) to anon;
grant execute on function public.get_coach_tl_submission_for_applicant(text) to anon;
grant execute on function public.get_coach_tl_submission_for_review(text) to anon;
grant execute on function public.get_coach_tl_submissions_for_admin(text, int) to anon;
grant execute on function public.set_coach_tl_submission_review_status(text, text) to anon;
