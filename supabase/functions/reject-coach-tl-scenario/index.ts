...
const DEFAULT_COACH_TL_POSITION_ID = "237";
const POSITION_LABEL = "Coach Team Leader";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const positionId =
      Deno.env.get("STARHIRE_COACH_TL_POSITION_ID") || DEFAULT_COACH_TL_POSITION_ID;

    const supabase = createClient(supabaseUrl, serviceRoleKey);
    const { data: submissionRow, error: submissionError } = await supabase
      .from("coach_tl_scenario_submissions")
      .select(
        [
          "id",
          "candidate_name",
